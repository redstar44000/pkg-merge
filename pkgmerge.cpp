...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "لطفاً پوشه حاوی فایل‌های .pkg را روی این فایل بیندازید.\n";
        return 1;
    }
    ...
    if (pkg_files.empty()) {
        std::cerr << "هیچ فایل .pkg پیدا نشد.\n";
        return 1;
    }
    ...
    if (!out) {
        std::cerr << "نمی‌توان فایل خروجی ساخت: " << out_path << '\n';
        return 1;
    }
