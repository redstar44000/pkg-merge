...
bool natural_compare(const fs::path& a, const fs::path& b) {
    ...
    std::sregex_iterator it_a(sa.begin(), sa.end(), re);
    std::sregex_iterator it_b(sb.begin(), sb.end(), re);
    ...
    return sa < sb; // fallback to string compare
}
...
void merge(map<string, Package> packages) {
	for (auto & root : packages) {
		...
		std::sort(pkg.parts.begin(), pkg.parts.end(), [](const Package& a, const Package& b) {
    return natural_compare(a.file, b.file);
});
